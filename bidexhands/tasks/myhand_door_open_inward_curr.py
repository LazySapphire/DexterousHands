...
class MyHandDoorOpenInward(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        shadow_hand_start_pose.r = gymapi.Quat().from_euler_zyx(0, -1.57, 0)
...
def compute_hand_reward(
    ...
    up_rew = torch.where(right_hand_finger_dist < 0.2,
                    torch.where(left_hand_finger_dist < 0.2,
    ...
    reward = - torch.max(right_hand_dist_rew, left_hand_dist_rew) + up_rew
    # reward = torch.clip(reward, min=-.5)
    ...
    resets = torch.where(right_hand_finger_dist >= 1., torch.ones_like(reset_buf), reset_buf)
