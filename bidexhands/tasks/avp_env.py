...
class IsaacVisualizerEnv: 
    ...
    def step(self, transformation: Dict[str, torch.Tensor], sync_frame_time = False): 
        ...
        if self.args.record:
            self.save_state(new_root_state)
    ...
    def save_state(self, new_root_state): 

        sim_right_wrist = new_root_state[1, :7]    # sim_right_wrist
        sim_right_fingers = new_root_state[3:28, :7] # sim_right_fingers
        right_wrist_fingers = torch.cat([sim_right_wrist.unsqueeze(0), sim_right_fingers], dim = 0)

        # Initialize saved_frames list if it doesn't exist
        if not hasattr(self, 'saved_frames'):
            self.saved_frames = []
        
        # Convert to numpy and append current frame to list
        right_wrist_fingers_np = right_wrist_fingers.cpu().numpy()
        self.saved_frames.append(right_wrist_fingers_np)

        # Save all frames to a single file
        save_path = CUR_PATH / "saved_frames"
        save_path.mkdir(exist_ok=True)
        frames_array = np.stack(self.saved_frames)  # Shape: [num_frames, 26, 7]
        np.save(save_path / "all_frames.npy", frames_array)
...
if __name__=="__main__": 
    ...
    parser.add_argument('--ip', type = str, default = "192.168.100.117")
