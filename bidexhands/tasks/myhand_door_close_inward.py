...
class MyHandDoorCloseInward(BaseTask):
    """
...
def compute_hand_reward(
    ...
    resets = torch.where(right_hand_finger_dist >= 2, torch.ones_like(reset_buf), reset_buf)
    resets = torch.where(left_hand_finger_dist >= 4, torch.ones_like(resets), resets)
